...
train_dataset = data_utils.FMData('data/'+config.dataset+'/train.txt', features_map)
valid_dataset = data_utils.FMData('data/'+config.dataset+'/valid.txt', features_map)
test_dataset = data_utils.FMData('data/'+config.dataset+'/test.txt', features_map)
...
feature_emb = torch.FloatTensor(np.load('data/'+config.dataset+'/feature_emb.npy')).cuda()
...
if config.model == 'FM':
	...
	model = model.NFM(
		...
		args.batch_norm, eval(args.dropout), FM_model, feature_emb)
model.cuda()
...
for epoch in range(args.epochs):
	...
	for features, feature_values, label in train_loader:
		...
		loss = criterion(prediction*100, label*100) 
	...
	if test_result < best_rmse:
		...
		if args.out:
			if not os.path.exists('models/'):
				os.mkdir('models/')
			torch.save(model, 
				'{}{}.pth'.format('models/', config.model))
